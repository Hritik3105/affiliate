                ...
                if emp_check:
                    for i in data_max:
                        ...
                        if sales_done[0]["salespaid"]:
                            ...
                            PaymentDetails.objects.filter(vendor=self.request.user.id,campaign=i["campaign_detail"],influencer=i["influencer"]).update(sales=cal_amt,influencerfee=i["influener_fee"],offer=i["offer"],amount=i["amount"],salespaid=0)
                        # else:
